		...
		$cesta=array();
				if($accion=="Agregar a Cesta"){
					if(!isset($_COOKIE['cesta'])){
							$cesta=array(array($matricula,$fechaActual));
							$codificar=serialize($cesta);
							setcookie('cesta',"$codificar", time() + (86400), "/");	
					}
					else{
							$datos=array($matricula,$fechaActual);//Nuevo vehiculo para agregar
							$cesta=unserialize($_COOKIE['cesta']);//Recuperamos el producto anterior en array
